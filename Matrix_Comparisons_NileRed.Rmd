---
title: "Microplastics Intercalibration Study Data Analysis: Matrix Comparisons, Nile Red"
...
Results_Clean <- Results %>%
  ...
  #Create factor for nile red
  mutate(nilered_f = as.factor(nilered)) %>% 
...
# Results are selected to only include particles that were identified via nile red. Counts for unique samples and particle IDs for total, size fraction, color and morphology are calculated and a new column is created for each.

#Calculate the number of particles for each characteristic and combination 

Results_Clean_NileRed <- Results_Clean %>%
  #Select particles analyzed by nile red
  filter(nilered_f == "Yes") %>% 
  ...
  ungroup() %>% 
  #Total particles by morphology within size fraction
  group_by(sampletype_f, sampleid, sizefraction_f, morphology_f) %>% 
  mutate(total_morphology_sizefraction = n_distinct(sampleid, particleid)) %>% 
  ungroup() %>% 
  #Select and rename relevant type of expertise to work for fields below
  rename("expertise" = expertisenilered) %>% 
...
#All of the following calculations distinguish between plastic and non-plastic spikes
  ...
  ungroup()%>%
  #Total particle spike per size fraction and morphology
  group_by(sampletype_f, sampleid, plastic_f, sizefraction_f, morphology_f) %>% 
  mutate(total_spike_morphology_sizefraction = sum(averagenumber, na.rm = TRUE)) %>% 
  ungroup() 
...
#Join relevant columns of reference data frame and results to compare expected counts to actual counts for nile red only data. Recovery for each category is calculated on a sample-specific basis. A new recovery column is created for each category. 
...
Join_NileRed_Total <- inner_join(Results_Clean_NileRed, totalspikecol, by = c("sampleid", "sampletype_f")) %>% #Join total_spike column to nile red results data frame
    mutate(total_pct_recovery = ((total/total_spike)*100)) %>%  #Create a new column for the total recovery of nile red only results
    group_by(labid, sampletype_f, sampleid, plastic_f, nilered_f, total, total_spike, total_pct_recovery, expertise) %>% 
    summarise()
...
sizefractionsumresults <- Results_Clean_NileRed %>% 
  ...
  group_by(labid, sampletype_f, sampleid, nilered_f, sizefraction_f, total_sizefraction, expertise) %>% 
  summarise()

Join_NileRed_SizeFraction <- inner_join(sizefractionsumresults, sizefractionspikecol, by = c("sampleid" = "sampleid", "sizefraction_f" = "sizefraction_f", "sampletype_f")) %>%  
...
morphologysumresults <- Results_Clean_NileRed %>% 
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blank samples from results
  group_by(labid, sampletype_f, sampleid, nilered_f, morphology_f, total_morphology, expertise) %>% 
  summarise()

Join_NileRed_Morphology <- inner_join(morphologysumresults, morphologyspikecol, by = c("sampleid" = "sampleid", "morphology_f" = "morphology_f", "sampletype_f")) %>%  
    mutate(morphology_pct_recovery = ((total_morphology/total_spike_morphology)*100))

#Combination - Size Fraction and Morphology 

 combospikecol <- Reference_Clean %>%
   distinct(sampletype_f, sampleid, plastic_f, sizefraction_f, morphology_f, total_spike_morphology_sizefraction)

 combosumresults <- Results_Clean_NileRed %>%
   filter(!grepl("4", sampleid)) %>%
   filter(!grepl("8", sampleid)) %>% #Dropping blank samples from results
   drop_na(sizefraction_f) %>% #Drop particles not assigned to a size fraction
   group_by(labid, sampletype_f, sampleid, nilered_f, sizefraction_f, morphology_f, total_morphology_sizefraction, expertise) %>%
   summarise()

 Join_NileRed_Combo <- inner_join(combosumresults, combospikecol, by = c("sampleid" = "sampleid", "morphology_f" = "morphology_f", "sizefraction_f" = "sizefraction_f", "sampletype_f")) %>%
   mutate(morphology_sizefraction_pct_recovery = ((total_morphology_sizefraction/total_spike_morphology_sizefraction)*100))
...
#morphology within size fraction
e <- Reference_Clean %>%
  distinct(sampletype_f, sampleid, plastic_f, morphology_f, sizefraction_f, total_spike_morphology_sizefraction) %>% 
  group_by(sampletype_f, plastic_f, morphology_f, sizefraction_f) %>% 
  mutate(total_spike_mean = mean(total_spike_morphology_sizefraction)) %>% 
  mutate(total_spike_sd = sd(total_spike_morphology_sizefraction)) %>%
  ungroup() %>% 
  group_by(sampletype_f, plastic_f, morphology_f, sizefraction_f, total_spike_mean, total_spike_sd) %>% 
  summarise() %>%
  rename("Matrix" = sampletype_f, "Plastic?" = plastic_f, "Morphology" = morphology_f, "Size Fraction" = sizefraction_f, 
         "Number of Spiked Particles" = total_spike_mean, "Standard Deviation" = total_spike_sd)

kable(e, digits = 2, caption = "Average Number of Spiked Particles per Sample by Morphology Within Size Fraction") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  scroll_box(width = "700px", height = "200px")
...
a <- Results_Clean_NileRed %>% 
  ...
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Particle Count", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Raw Particle Count", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line represents the number of spiked polymer particles.\nNot blank corrected.")

plot(a)

b <- Results_Clean_NileRed %>% 
  filter(blank != "Y") %>% 
...
#Total - Summary
c <- Results_Clean_NileRed %>%  
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blanks
  group_by(labid, sampleid, nilered_f, total, sampletype_f) %>% 
  summarise() %>% 
  group_by(labid) %>% 
  mutate(total_mean = mean(total)) %>% 
  ungroup() %>% 
  group_by(labid, nilered_f, total_mean, sampletype_f) %>% 
  summarise() %>%   
  #Creating plot
  ggplot(aes(x=nilered_f, y= total_mean)) +
  theme_test() +
  geom_point(aes(color = nilered_f, fill = nilered_f, alpha = 0.7), width = .05, size = 3) +
  geom_hline(data = spike, aes(yintercept = mean_spike), linetype = "dotted")+
  geom_boxplot(alpha = 0.5,aes(color = nilered_f, fill = nilered_f), outlier.shape = NA) +
  theme(text = element_text(size = 12),
        plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
  ...
  facet_wrap(~sampletype_f, ncol = 2)+
  expand_limits(y = 0) +
  labs(y = "Particle Count", fill = "Lab", title = "NileRed", subtitle = "Raw Particle Count", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line represents the number of spiked polymer particles.\nNot blank corrected.")
  
plot(c)

d <- Results_Clean_NileRed %>% 
  filter(blank != "Y") %>% 
...
a <- Join_NileRed_Total %>%
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue", "orchid"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Total Particle Recovery", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(a)

#Total - By Lab
a_a <- Join_NileRed_Total %>%
  filter(sampletype_f != "Dirty Water") %>% 
  filter(plastic_f != "N") %>% 
  ungroup() %>% 
  group_by(labid) %>% 
  ...
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Matrix", color = "Matrix", title = "NileRed", subtitle = "Total Particle Recovery", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(a_a)

b <- Join_NileRed_Total %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
...
kable(b, digits = 2, caption = "NileRed: Total Particle Recovery. Summary statistics by sample (n = 1-3 samples). Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")

#Total - Summary
c<- Join_NileRed_Total %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, total_pct_recovery) %>% 
  summarise() %>% 
  group_by(sampletype_f, labid) %>% 
  mutate(total_pct_recovery_mean = mean(total_pct_recovery)) %>% 
  ungroup() %>% 
  group_by(sampletype_f, labid, nilered_f, total_pct_recovery_mean) %>% 
  summarise() %>% 
  #Creating plot
  ggplot(aes(x=nilered_f, y= total_pct_recovery_mean)) +
  theme_test() +
  geom_rect(ymin = 95.7, ymax = 104.3, xmin = 0, xmax = Inf, color = "gray87", fill = "gray87", alpha = 0.5)+
  geom_point(aes(color = nilered_f, fill = nilered_f, alpha = 0.7), width = .05, size = 3) +
  geom_boxplot(alpha = 0.5,aes(color = nilered_f, fill = nilered_f), outlier.shape = NA) +
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Total Particle Recovery", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(c)

d <- Join_NileRed_Total %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  group_by(sampletype_f, labid) %>% 
...
kable(d, digits = 2, caption = "NileRed: Total Particle Recovery. Summary statistics by laboratory (n = 1-3 samples). Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")

e <- Join_NileRed_Total %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  group_by(sampletype_f, labid) %>% 
  ...
  ungroup() %>% 
  rename("Matrix" = sampletype_f) %>% 
  select(`Matrix`, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  distinct() 

kable(e, digits = 2, caption = "NileRed: Total Particle Recovery. Summary statistics across all laboratories. Not blank corrected.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")
```
...
a <- Join_NileRed_SizeFraction %>%  
  ...
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, sizefraction_f, sizefrac_pct_recovery) %>% 
  summarise() %>% 
  group_by(sampletype_f, labid, sizefraction_f) %>% 
  mutate(sizefrac_pct_recovery_mean = mean(sizefrac_pct_recovery)) %>% 
  ungroup() %>% 
  group_by(sampletype_f, labid, nilered_f, sizefraction_f, sizefrac_pct_recovery_mean) %>% 
  summarise() %>% 
  #Creating plot
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, All Size Fractions", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(a)

#1-20µm - By Lab
b <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Particle Recovery, 1-20 µm", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(b)

#1-20µm - Summary
c <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, sizefraction_f, sizefrac_pct_recovery) %>% 
  ...
  ungroup() %>% 
  filter(sizefraction_f == "1-20 µm") %>%
  group_by(sampletype_f, labid, nilered_f, sizefrac_pct_recovery_mean) %>% 
  summarise() %>% 
  #Creating plot
  ggplot(aes(x=nilered_f, y= sizefrac_pct_recovery_mean)) +
  theme_test() +
  #No RSD data for the 1-20 µm size fraction
  geom_point(aes(color = nilered_f, fill = nilered_f, alpha = 0.7), width = .05, size = 3) +
  geom_boxplot(alpha = 0.5,aes(color = nilered_f, fill = nilered_f), outlier.shape = NA) +
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, 1-20 µm", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(c)

#20-212 µm - By Lab
d <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Particle Recovery, 20-212 µm", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(d)

#20-212 µm - Summary
e <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, sizefraction_f, sizefrac_pct_recovery) %>% 
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, 20-212 µm", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(e)

#212-500 µm - By Lab
f <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Particle Recovery, 212-500 µm", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(f)

#212-500 µm - Summary
g <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, sizefraction_f, sizefrac_pct_recovery) %>% 
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, 212-500 µm", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(g)

#>500 µm - By Lab
h <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Particle Recovery, >500 µm", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(h)

#>500 µm - Summary
i <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, sizefraction_f, sizefrac_pct_recovery) %>% 
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, >500 µm", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(i)

#Size Fraction - By Sample
j <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
...
kable(j, digits = 2, caption = "NileRed: Total Particle Recovery by Size Fraction. Summary statistics by sample (n = 1-3 samples). Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")

#Size Fraction - By Lab
k <- Join_NileRed_SizeFraction %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>% 
  group_by(sampletype_f, labid, sizefraction_f) %>% 
  ...
  ungroup() %>% 
  select(sampletype_f, labid, sizefraction_f, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  rename("Matrix" = sampletype_f) %>%
  rename("Lab ID" = labid) %>%
  rename("Size Fraction" = sizefraction_f) %>%
  distinct() %>% 
...
kable(k, digits = 2, caption = "NileRed: Total Particle Recovery by Size Fraction. Summary statistics by laboratory (n = 1-3 samples). Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")
...
l <- k %>% 
  ...
  mutate(`Max (%)` = max(`Mean (%)`)) %>%
  ungroup() %>% 
  select(`Matrix`, `Size Fraction`, `Global Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  distinct() %>% 
  arrange(`Matrix`,`Size Fraction`)

kable(l, digits = 2, caption = "NileRed: Total Particle Recovery by Size Fraction. Summary statistics across all laboratories. Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")
...
#All Morphologies
a <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, morphology_f, morphology_pct_recovery) %>% 
  summarise() %>% 
  group_by(sampletype_f, labid, morphology_f) %>% 
  mutate(morphology_pct_recovery_mean = mean(morphology_pct_recovery)) %>% 
  ungroup() %>% 
  group_by(sampletype_f, labid, nilered_f, morphology_f, morphology_pct_recovery_mean) %>% 
  summarise() %>% 
  #Creating plot
  ggplot(aes(x=sampletype_f, y= morphology_pct_recovery_mean)) +
  theme_test() +
  geom_point(aes(color = nilered_f, fill = nilered_f, alpha = 0.7), width = .05, size = 3) +
  geom_boxplot(alpha = 0.7,aes(color = nilered_f, fill = nilered_f), outlier.shape = NA) +
  geom_hline(yintercept = 100, linetype = "dotted")+
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, All Morphologies", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(a)


#Fibers - By Lab
b <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Particle Recovery, Fibers", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(b)

#Fiber - Summary
c <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, morphology_f, morphology_pct_recovery) %>% 
  ...
  ungroup() %>% 
  filter(morphology_f == "Fiber") %>%
  group_by(sampletype_f, labid, nilered_f,morphology_pct_recovery_mean) %>% 
  summarise() %>% 
  #Creating plot
  ggplot(aes(x=nilered_f, y= morphology_pct_recovery_mean)) +
  theme_test() +
  #No RSD data for fibers
  geom_point(aes(color = nilered_f, fill = nilered_f, alpha = 0.7), width = .05, size = 3) +
  geom_boxplot(alpha = 0.5,aes(color = nilered_f, fill = nilered_f), outlier.shape = NA) +
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, Fibers", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(c)

#Spheres - By Lab
d <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Particle Recovery, Spheres", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(d)

#Sphere - Summary
e <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, morphology_f, morphology_pct_recovery) %>% 
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, Sphere", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(e)

#Fragments - By Lab
f <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "NileRed", subtitle = "Particle Recovery, Fragments", caption = "Points represent individual samples spiked with microplastic particles.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(f)

#Fragments - Summary
g <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, nilered_f, morphology_f, morphology_pct_recovery) %>% 
  ...
  geom_rect(ymin = 93.6, ymax = 106.4, xmin = 0, xmax = Inf, color = "gray87", fill = "gray87", alpha = 0.5)+
  geom_point(aes(color = nilered_f, fill = nilered_f, alpha = 0.7), width = .05, size = 3) +
  geom_boxplot(alpha = 0.5,aes(color = nilered_f, fill = nilered_f), outlier.shape = NA) +facet_wrap(~sampletype_f, ncol = 3)+
  geom_hline(yintercept = 100, linetype = "dotted")+
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Lab", title = "NileRed", subtitle = "Particle Recovery, Fragments", caption = "Points represent the sample mean (n = 1-3) of each laboratory.\nDotted line and shaded area represent 100% recovery of polymer materials and RSD, respectively.\nNot blank corrected.")

plot(g)

#Morphology - By Sample
h <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
...
kable(h, digits = 2, caption = "NileRed: Total Particle Recovery by Size Fraction. Summary statistics by sample (n = 1-3 samples). Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")

#Morphology - By Lab
i <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  group_by(sampletype_f, labid, morphology_f) %>% 
  ...
  ungroup() %>% 
  select(sampletype_f, labid, morphology_f, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  rename("Matrix" = sampletype_f) %>% 
  rename("Lab ID" = labid) %>%
  rename("Morphology" = morphology_f) %>%
  distinct() 

kable(i, digits = 2, caption = "NileRed: Total Particle Recovery by Morphology. Summary statistics by laboratory (n = 1-3 samples). Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px") 

#Morphology - Summary
j <- Join_NileRed_Morphology %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ...
  mutate(`Max (%)` = max(morphology_pct_recovery)) %>%
  ungroup() %>% 
  select(sampletype_f, morphology_f, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  rename("Matrix" = sampletype_f) %>%
  rename("Morphology" = morphology_f) %>%
  distinct() 

kable(j, digits = 2, caption = "NileRed: Total Particle Recovery by Morphology. Summary statistics across all laboratories. Not blank corrected.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")
```
...
The following plots display the recovery (reported count/spiked count (%)) obtained from microscopy with nile red. **Reported particle characteristics must match expected particle characteristics of the spiking materials to count towards recovery estimations.** For example, the morphologies of polymer spiking materials are defined as spheres, fragments, and fibers. Morphologies outside of these definitions are currently excluded from analysis. 

The dotted line is at 100% and the grey area represents the mean RSD for each given category. The data are **not** blank corrected.

Recovery in this section is estimated based on color and morphology within a size fraction. For example, if the sample was spiked with 10 spheres >500 µm and 8 spheres were recovered in the >500 µm size fraction, recovery is estimated as 80%. However, if 4 of these spheres were reported as pellets rather than spheres, recovery would fall to 60%. 

```{r echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, results = 'asis'}

#Make data frame of particle characteristics
df <- Join_NileRed_Combo %>%
  filter(sampletype_f != "Dirty Water") %>% 
  filter(plastic_f == "Y") %>% 
  group_by(sampletype_f, morphology_f, sizefraction_f) %>% 
  summarise()
...
#By Lab
a <- Join_NileRed_Combo %>%
  filter(plastic_f != "N") %>%
  filter(sampletype_f != "Dirty Water") %>% 
  group_by(labid, sampleid, sizefraction_f, morphology_f, morphology_sizefraction_pct_recovery) %>% 
  filter(sizefraction_f == df$sizefraction_f[[i]]) %>% 
  filter(morphology_f == df$morphology_f[[i]]) %>%
  ungroup() %>% 
  mutate(labid = fct_reorder(labid, expertise)) %>%  
  #Creating plot
  ggplot(aes(x=labid, y= morphology_sizefraction_pct_recovery)) +
  theme_test() +
  geom_point(aes(color = as.factor(expertise), fill = as.factor(expertise)), alpha = 0.7, width = .05, size = 3) +
  geom_hline(yintercept = 100, linetype = "dotted")+
  stat_summary(fun="mean", geom="segment", mapping=aes(xend=..x.. - 0.25, yend=..y..))+
  ...
  scale_color_manual(values = c("royalblue", "darkcyan", "darkblue", "orchid"))+
  expand_limits(y = 0) +
  labs(x = "Labs", y = "Recovery (%)", fill = "Experience", color = "Experience", title = "Nile Red", 
       subtitle = paste(df$sizefraction_f[[i]], df$morphology_f[[i]]), 
       caption = "Points represent individual samples spiked with microplastic particles.\nDotted line represents 100% recovery of polymer materials.\nNot blank corrected.")

plot(a)

#Summary
b <- Join_NileRed_Combo %>%
  filter(plastic_f != "N") %>%
  filter(sampletype_f != "Dirty Water") %>% 
  filter(sizefraction_f == df$sizefraction_f[[i]]) %>% 
  filter(morphology_f == df$morphology_f[[i]]) %>%
  group_by(labid) %>%
  mutate(morphology_sizefraction_pct_recovery_mean = mean(morphology_sizefraction_pct_recovery)) %>%
  ungroup() %>%
  group_by(sampletype_f, labid, nilered_f, morphology_sizefraction_pct_recovery_mean) %>%
  summarise() %>%  
  #Creating plot
  ggplot(aes(x=nilered_f, y= morphology_sizefraction_pct_recovery_mean)) +
  theme_test() +
  geom_point(aes(color = nilered_f, fill = nilered_f), alpha = 0.7, width = .05, size = 3) +
  geom_hline(yintercept = 100, linetype = "dotted")+
  geom_boxplot(alpha = 0.5,aes(color = nilered_f, fill = nilered_f), outlier.shape = NA) +
  ...
  scale_fill_manual(values = c("darkmagenta")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", title = "Nile Red", 
       subtitle = paste(df$sizefraction_f[[i]], df$morphology_f[[i]]),
...
#By Sample
c <- Join_NileRed_Combo %>%
  filter(plastic_f != "N") %>%
  filter(sampletype_f != "Dirty Water") %>% 
  group_by(sampletype_f, labid, sizefraction_f, morphology_f) %>%
  filter(sizefraction_f == df$sizefraction_f[[i]]) %>% 
  filter(morphology_f == df$morphology_f[[i]]) %>%
  ungroup() %>%
  select(sampletype_f, labid, sampleid, morphology_sizefraction_pct_recovery) %>%
  rename("Matrix" = sampletype_f) %>% 
  rename("Lab ID" = labid) %>%
  rename("Sample ID" = sampleid) %>%
  rename("Recovery" = morphology_sizefraction_pct_recovery) %>%
  distinct()

print(kable(c, digits = 2, caption = paste(df$sizefraction_f[[i]], df$morphology_f[[i]])) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  scroll_box(width = "700px", height = "200px"))
  cat('\n')

#By Lab
d <- Join_NileRed_Combo %>%
  filter(plastic_f != "N") %>%
  filter(sampletype_f != "Dirty Water") %>% 
  group_by(sampletype_f, labid, sizefraction_f, morphology_f) %>%
  filter(sizefraction_f == df$sizefraction_f[[i]]) %>% 
  filter(morphology_f == df$morphology_f[[i]]) %>%
  mutate(Mean = mean(morphology_sizefraction_pct_recovery)) %>%
  mutate(Median = median(morphology_sizefraction_pct_recovery)) %>%
  mutate(SD = sd(morphology_sizefraction_pct_recovery)) %>%
  mutate(Min = min(morphology_sizefraction_pct_recovery)) %>%
  mutate(Max = max(morphology_sizefraction_pct_recovery)) %>%
  ungroup() %>%
  select(sampletype_f, labid, Mean, Median, SD, Min, Max) %>%
  rename("Matrix" = sampletype_f) %>% 
  rename("Lab ID" = labid) %>%
...
print(kable(d, digits = 2, caption = paste(df$sizefraction_f[[i]], df$morphology_f[[i]])) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  scroll_box(width = "700px", height = "200px"))  
  cat('\n')
...
#### All Spiked Particles Summary ####
Combo_Microscopy_Summary_Table <- Join_NileRed_Combo %>%
  filter(plastic_f != "N") %>%
  filter(sampletype_f != "Dirty Water") %>% 
  group_by(labid, sizefraction_f, morphology_f) %>%
  mutate(Mean = mean(morphology_sizefraction_pct_recovery)) %>%
  mutate(Median = median(morphology_sizefraction_pct_recovery)) %>%
  mutate(SD = sd(morphology_sizefraction_pct_recovery)) %>%
  mutate(Min = min(morphology_sizefraction_pct_recovery)) %>%
  mutate(Max = max(morphology_sizefraction_pct_recovery)) %>%
  ungroup() %>%
  group_by(sizefraction_f, morphology_f) %>%
  ...
  mutate(`Max (%)` = max(Mean)) %>%
  ungroup() %>%
  select(sizefraction_f, morphology_f, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  rename("Size Fraction" = sizefraction_f) %>%
  rename("Morphology" = morphology_f) %>%
  distinct()

kable(Combo_Microscopy_Summary_Table, digits = 2, caption = "Nile Red: Recovery, All Spiked Particles. Summary statistics across all laboratories. Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  scroll_box(width = "700px", height = "200px")
...
df <- Join_NileRed_Combo %>%
  filter(plastic_f == "Y") %>% 
  group_by(morphology_f, sizefraction_f) %>% 
  summarise()
...
#By Lab
a <- Join_NileRed_Combo %>%
  group_by(labid, sampleid, nilered_f,sizefraction_f, morphology_f, morphology_sizefraction_pct_recovery) %>% 
  filter(sizefraction_f == df$sizefraction_f[[i]]) %>%
  filter(morphology_f == df$morphology_f[[i]]) %>%
  ungroup() %>% 
  mutate(labid = fct_reorder(labid, expertise)) %>%  
...
b <- Join_NileRed_Combo %>%
  ...
  mutate(morphology_sizefraction_pct_recovery_mean = mean(morphology_sizefraction_pct_recovery)) %>%
  ungroup() %>%
  group_by(labid, nilered_f,morphology_sizefraction_pct_recovery_mean) %>%
  summarise() %>%
  #Creating plot
  ggplot(aes(x=nilered_f,y= morphology_sizefraction_pct_recovery_mean)) +
  theme_test() +
...
c <- Join_NileRed_Combo %>%
  ...
  filter(morphology_f == df$morphology_f[[i]]) %>%
  ungroup() %>%
  select(labid, sampleid, morphology_sizefraction_pct_recovery) %>%
  rename("Lab ID" = labid) %>%
  rename("Sample ID" = sampleid) %>%
  rename("Recovery" = morphology_sizefraction_pct_recovery) %>%
  distinct()
...
d <- Join_NileRed_Combo %>%
  ...
  mutate(Max = max(morphology_sizefraction_pct_recovery)) %>%
  ungroup() %>%
  select(labid, Mean, Median, SD, Min, Max) %>%
  rename("Lab ID" = labid) %>%
  rename("Mean (%)" = Mean) %>%
  rename("Median (%)" = Median) %>%
  rename("SD (%)" = SD) %>%
