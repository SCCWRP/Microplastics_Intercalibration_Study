...
```{r Manuscript Figure 3, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

Fig3 <- Results_Clean_Microscopy %>%
  filter(!(labid == "Z"&sampletype_f == "Clean Water")) %>%
  filter(!(labid == "M"&sampletype_f == "Dirty Water")) %>%
  filter(!(labid == "M"&sampletype_f == "Fish Tissue")) %>%
  filter(!(labid == "M"&sampletype_f == "Sediment")) %>%
...
saveRDS(Fig3, file = "./Manuscript_Figures/Fig3")
...
```{r Manuscript Fig S3, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#All Size Fractions
FigS3 <- Join_Microscopy_SizeFraction %>%  
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ungroup() %>%
...
saveRDS(FigS3, file = "./Manuscript_Figures/FigS3")
...
```{r Manuscript Fig S4, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#All Colors
FigS4 <- Join_Microscopy_Color %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
...
saveRDS(FigS4, file = "./Manuscript_Figures/FigS4")
...
```{r Manuscript Fig S5, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

FigS5 <- Join_Microscopy_Morphology %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
...
saveRDS(FigS5, file = "./Manuscript_Figures/FigS5")
...
```{r Manuscript Fig 2, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#Total - Summary
Fig2<- Join_Microscopy_Total %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
    sampletype_f == "Clean Water" ~ "Drinking Water",
...
saveRDS(Fig2, file = "./Manuscript_Figures/Fig2")

```

```{r Manuscript Fig S2, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#Total - By Lab
FigS2 <- Join_Microscopy_Total %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  ungroup() %>% 
  mutate(sampletype_f = case_when(
...
```{r Manuscript Fig S6, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#All Size Fractions
FigS6 <- Join_Microscopy_SizeFraction %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  ungroup() %>%
  mutate(sampletype_f = case_when(
...
saveRDS(FigS6, file = "./Manuscript_Figures/FigS6")
...
FigS8 <- Join_Microscopy_Morphology %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
    sampletype_f == "Clean Water" ~ "Drinking Water",
...
# Performance By Sample Number

```{r echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, results = 'asis', fig.height = 10}

library(ggpubr)

df <- Join_Microscopy_Total %>% 
  group_by(sampletype_f) %>% 
  summarise()
...
a <- Join_Microscopy_Total %>%
  filter(sampletype_f == df$sampletype_f[[i]],
         plastic_f == "Y") %>%
  mutate(sample_number = as.numeric(str_extract(sampleid, "\\d"))) %>% 
  group_by(labid) %>% 
  filter(n()>2)
  
b <- ggscatter(a, x = "sample_number", y = "total_pct_recovery", 
            add = "reg.line", conf.int = TRUE, 
            cor.coef = TRUE, cor.method = "spearman")+ 
     facet_wrap(~labid, scales = "free")+
     theme_test()+
     labs(x = "Sample Number", y = "Recovery (%)", title = paste(df$sampletype_f[[i]]),
          subtitle = "Spearman's Rank-Order Correlation of Recovery by Sample Number within Lab")
