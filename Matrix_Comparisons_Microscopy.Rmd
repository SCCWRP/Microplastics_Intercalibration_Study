---
title: "Microplastics Intercalibration Study Data Analysis: Matrix Comparisons, Microscopy"
...
c <- Results_Clean_Microscopy %>%  
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blanks
  group_by(labid, set, sampleid, microscopy_f, total, sampletype_f) %>% 
  summarise() %>% 
  group_by(labid) %>% 
...
#Size Fraction Count
f <- Results_Clean_Microscopy %>%  
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blanks
  filter(sampletype_f != "Dirty Water") %>% 
  group_by(sampletype_f, sizefraction_f, labid, set, total_sizefraction) %>% 
  summarise() %>% 
  mutate(total_mean = mean(total_sizefraction)) %>% 
  ungroup() %>% 
  distinct(sampletype_f, sizefraction_f, labid, total_mean) %>%
  ...
  mutate(`Max` = max(total_mean)) %>%
  ungroup() %>% 
  select(sampletype_f, sizefraction_f, `Mean`, `Median`, `SD`, `Min`, `Max`) %>%
  rename("Matrix" = sampletype_f) %>%
  rename("Size Fraction" = sizefraction_f) %>%
  distinct() 

kable(f, digits = 2, caption = "Raw Particle Count: Size Fraction. Global summary statistics. Not blank corrected.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")

#Color Count
g <- Results_Clean_Microscopy %>%  
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blanks
  filter(sampletype_f != "Dirty Water") %>% 
  group_by(sampletype_f, color_f, labid, set, total_color) %>% 
  summarise() %>% 
  mutate(total_mean = mean(total_color)) %>% 
  ungroup() %>% 
  distinct(sampletype_f, color_f, labid, total_mean) %>%
  ...
  mutate(`Max` = max(total_mean)) %>%
  ungroup() %>% 
  select(sampletype_f, color_f, `Mean`, `Median`, `SD`, `Min`, `Max`) %>%
  rename("Matrix" = sampletype_f) %>%
  rename("Color" = color_f) %>%
  distinct() 

kable(g, digits = 2, caption = "Raw Particle Count: Color. Global summary statistics. Not blank corrected.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")

#Morphology Count
h <- Results_Clean_Microscopy %>%  
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blanks
  filter(sampletype_f != "Dirty Water") %>% 
  group_by(sampletype_f, morphology_f, labid, set, total_morphology) %>% 
  summarise() %>% 
  mutate(total_mean = mean(total_morphology)) %>% 
  ungroup() %>% 
  distinct(sampletype_f, morphology_f, labid, total_mean) %>%
...
kable(h, digits = 2, caption = "Raw Particle Count: Morphology. Global summary statistics. Not blank corrected.")%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")
...
#Total - By Lab
a <- Results_Clean_Microscopy %>%
  filter(!(labid == "Z"&sampletype_f == "Clean Water")) %>%
  filter(!(labid == "M"&sampletype_f == "Dirty Water")) %>%
  filter(!(labid == "M"&sampletype_f == "Fish Tissue")) %>%
  filter(!(labid == "M"&sampletype_f == "Sediment")) %>%
  #Data manipulation
  filter(blank == "Y") %>% 
  group_by(labid, sampleid, total, expertise, sampletype_f) %>% 
  summarise() %>% 
  ...
  facet_wrap(~sampletype_f, ncol = 2)+
  expand_limits(y = 0) +
  labs(x = "Lab", y = "Particle Count", title = "Microscopy", subtitle = "Blank Particle Count")
...
kable(b, digits = 2, caption = "Blank Particle Count.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  scroll_box(width = "700px", height = "200px")
...
# shapiro.test(log10(blank_stats$`Total Particles`)) #Normal distribution after log transformation
# bartlett.test(log10(`Total Particles`) ~ Matrix, data = blank_stats) #Equal variance after log transformation
# x <- aov(log10(`Total Particles`) ~ Matrix, data = blank_stats) #p = 0.00733
# summary(x)
# TukeyHSD(x) #Significant differences between clean and dirty water

kruskal.test(`Total Particles` ~ Matrix, data = blank_stats)
Steel.Dwass(blank_stats$`Total Particles`, as.character(blank_stats$Matrix))
...
c <- Results_Clean_Microscopy %>%
  ...
  filter(!(labid == "M"&sampletype_f == "Sediment")) %>%
  filter(blank == "Y") %>% 
  group_by(labid, sampleid, total, sampletype_f) %>% 
  summarise() %>% 
  ungroup() %>% 
  #Creating plot
  ggplot(aes(x=sampletype_f, y= total)) +
  theme_test() +
  geom_point(aes(color = sampletype_f, fill = sampletype_f, alpha = 0.7), size = 3) +
  ...
  scale_fill_manual(values = c("royalblue", "forestgreen", "darksalmon", "goldenrod4")) +
  expand_limits(y = 0) +
  labs(y = "Particle Count", title = "Microscopy", subtitle = "Blank Particle Count")
...
d <- Results_Clean_Microscopy %>%
  ...
  filter(!(labid == "M"&sampletype_f == "Sediment")) %>%
  filter(blank == "Y") %>% 
  group_by(labid, sampletype_f, total) %>% 
  summarise() %>% 
  ungroup() %>%
  group_by(sampletype_f) %>% 
  mutate(`Mean`= mean(total)) %>%  
...
kable(d, digits = 2, caption = "Blank Particle Count.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  scroll_box(width = "700px", height = "200px")

```

```{r Manuscript Figure S2, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

FigS2 <- Results_Clean_Microscopy %>%
  filter(!(labid == "Z"&sampletype_f == "Clean Water")) %>%
  filter(!(labid == "M"&sampletype_f == "Dirty Water")) %>%
  filter(!(labid == "M"&sampletype_f == "Fish Tissue")) %>%
  filter(!(labid == "M"&sampletype_f == "Sediment")) %>%
  ...
  mutate(sampletype_f = case_when(
    ...
    sampletype_f == "Dirty Water" ~ "Surface Water",
  ...
  theme(text = element_text(size = 20),
        ...
        axis.title.x = element_blank(),
        axis.ticks.x = element_blank())+
  scale_color_manual(values = c("royalblue", "darksalmon", "goldenrod4", "forestgreen")) +
  scale_fill_manual(values = c("royalblue", "darksalmon", "goldenrod4", "forestgreen")) +
  expand_limits(y = 0) +
  labs(y = "Particle Count")
  
plot(FigS2)

saveRDS(FigS2, file = "./Manuscript_Figures/FigS2")
...
```{r Manuscript Fig S7, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#All Size Fractions
FigS7 <- Join_Microscopy_SizeFraction %>%  
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  ungroup() %>%
  mutate(sampletype_f = case_when(
    ...
    sampletype_f == "Sediment" ~ "Sediment")) %>%
  #Force size fractions to be in the correct order
  mutate(sizefraction_f = factor(sizefraction_f, levels = c("1-20 µm", "20-212 µm","212-500 µm",">500 µm"))) %>%   #Data manipulation
  group_by(sampletype_f, labid, set, sampleid, microscopy_f, sizefraction_f, sizefrac_pct_recovery) %>% 
  summarise() %>% 
  group_by(sampletype_f, labid, sizefraction_f) %>% 
  mutate(sizefrac_pct_recovery_mean = mean(sizefrac_pct_recovery)) %>% 
  ...
  geom_boxplot(alpha = 0.5,aes(color = sampletype_f, fill = sampletype_f), outlier.shape = NA) +
  geom_hline(yintercept = 100, linetype = "dotted")+
  facet_wrap(~sizefraction_f, nrow = 1, scales = "free")+
  theme(text = element_text(size = 20),
        legend.position = "right",
        axis.text.x = element_blank(),
        axis.title.x = element_blank(),
  ...
  scale_fill_manual(values = c("royalblue", "darksalmon", "goldenrod4")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", fill = "Matrix", color = "Matrix")

plot(FigS7)

saveRDS(FigS7, file = "./Manuscript_Figures/FigS7")
...
m <- Join_Microscopy_Color %>%
  ...
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  group_by(sampletype_f, labid, color_f, set) %>% 
...
```{r Manuscript Fig S8, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#All Colors
FigS8 <- Join_Microscopy_Color %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
  ...
  geom_boxplot(alpha = 0.5,aes(color = sampletype_f, fill = sampletype_f), outlier.shape = NA) +
  geom_hline(yintercept = 100, linetype = "dotted")+
  facet_wrap(~color_f, nrow = 1, scales = "free")+
  theme(text = element_text(size = 20),
        legend.position = "right",
        axis.text.x = element_blank(),
        axis.title.x = element_blank(),
...
saveRDS(FigS8, file = "./Manuscript_Figures/FigS8")
...
```{r Manuscript Fig S9, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

FigS9 <- Join_Microscopy_Morphology %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
  ...
  theme_test() +
  geom_point(aes(color = sampletype_f, fill = sampletype_f), alpha = 0.7, size = 3) +
  geom_boxplot(alpha = 0.7,aes(color = sampletype_f, fill = sampletype_f), outlier.shape = NA) +
  geom_hline(yintercept = 100, linetype = "dotted")+
  facet_wrap(~morphology_f, ncol = 3, scales = "free")+
  theme(text = element_text(size = 20),
        legend.position = "right",
...
saveRDS(FigS9, file = "./Manuscript_Figures/FigS9")
...
c<- Join_Microscopy_Total %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
  filter(!(labid %in% c("AA", "L", "M", "Q", "R"))) %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, microscopy_f, total_pct_recovery) %>% 
  summarise() %>% 
  group_by(sampletype_f, labid) %>% 
  mutate(total_pct_recovery_mean = mean(total_pct_recovery)) %>% 
  ungroup() %>% 
...
d <- Join_Microscopy_Total %>%
  ...
  ungroup() %>%
  group_by(sampletype_f) %>% 
  mutate(mean_se = mean(`SE (%)`, na.rm = TRUE)) %>% 
  select(sampletype_f, labid, `Mean (%)`, `Median (%)`, `SD (%)`, `SE (%)`, `Min (%)`, `Max (%)`, mean_se) %>%
  rename("Matrix" = sampletype_f) %>%  
  rename("Lab ID" = labid) %>%
  distinct() 
...
```{r Manuscript Fig 1C, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#Total - Summary
Fig1C<- Join_Microscopy_Total %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
    sampletype_f == "Clean Water" ~ "Drinking Water",
  ...
  scale_fill_manual(values = c("royalblue", "darksalmon", "goldenrod4", "forestgreen")) +
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", title = "> 212 µm")

plot(Fig1C)

saveRDS(Fig1C, file = "./Manuscript_Figures/Fig1C")

```

```{r Manuscript Fig S10, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#Total - By Lab
FigS10 <- Join_Microscopy_Total %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  ungroup() %>% 
  mutate(sampletype_f = case_when(
  ...
  scale_color_manual(values = c("royalblue", "darksalmon", "goldenrod4", "forestgreen"))+
  expand_limits(y = 0) +
  labs(y = "Recovery (%)", x = "Lab", title = "> 212 µm")

plot(FigS10)

saveRDS(FigS10, file = "./Manuscript_Figures/FigS10")
...
```{r Manuscript Fig S11, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#All Size Fractions
FigS11 <- Join_Microscopy_SizeFraction %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  ungroup() %>%
  mutate(sampletype_f = case_when(
...
saveRDS(FigS11, file = "./Manuscript_Figures/FigS11")
...
```{r Manuscript Fig S12, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#All Colors
FigS12 <- Join_Microscopy_Color %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
    sampletype_f == "Clean Water" ~ "Drinking Water",
...
saveRDS(FigS12, file = "./Manuscript_Figures/FigS12")
...
```{r Manuscript Fig S13, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

FigS13 <- Join_Microscopy_Morphology %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
    sampletype_f == "Clean Water" ~ "Drinking Water",
...
saveRDS(FigS13, file = "./Manuscript_Figures/FigS13")
