...
Results_Clean <- Results %>%
  ...
  filter(!(sizefraction_f %in% c("1-20 µm", "10-20 µm", "1-10 µm", "1-250 µm")))
...
```{r Manuscript Fig 1B, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#Total - Summary
Fig1B<- Join_Microscopy_Total %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>%
  filter(plastic_f != "N") %>%
...
saveRDS(Fig1B, file = "./Manuscript_Figures/Fig1B")

```

```{r Manuscript Figure S5B, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#Total - By Lab
FigS5B <- Join_Microscopy_Total %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>% 
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
...
saveRDS(FigS5B, file = "./Manuscript_Figures/FigS5B")
