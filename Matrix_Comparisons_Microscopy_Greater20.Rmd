...
```{r Manuscript Figure S1A, echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, include = FALSE}

#Total - By Lab
FigS1A <- Join_Microscopy_Total %>%
  filter(!(labid == "R"&sampletype_f == "Clean Water")) %>%
  filter(sampletype_f != "Dirty Water") %>% 
  filter(plastic_f != "N") %>%
  mutate(sampletype_f = case_when(
...
saveRDS(FigS1A, file = "./Manuscript_Figures/FigS1A")
