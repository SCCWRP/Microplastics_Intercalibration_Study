...
```{r echo = FALSE, results = 'hide', error = FALSE, warning = FALSE, message = FALSE, include = FALSE}
...
Fig4 <-  Raman_FTIR %>%
   ...
   ggplot(aes(x = method, y = prop, fill = sampletype_f)) + 
   theme_test() +
   geom_col(aes(alpha = correct), stat = 'identity', 
            position = position_fill(reverse = TRUE), color = "black") +
   geom_text(aes(y = label_y, label = n), size = 4.5)+
   facet_wrap(~sampletype_f, ncol = 4)+
   ylim(0,1)+
   ...
   scale_fill_manual(values = c("royalblue", "darksalmon", "goldenrod4", "forestgreen"), guide = FALSE) +
   scale_y_continuous(labels = scales::percent, expand = c(0,0), limits = c(0,1.02))+
   labs(x = 'Method', y = "Proportion of Particles\nCorrectly Identified", alpha = "Identification") +
   scale_alpha_manual(values = c(1,0.25))
   
Fig4

saveRDS(Fig4, file = "./Manuscript_Figures/Fig4")
...
FigS9 <- Results_Spike_FTIR %>%
   ...
   labs(title = "FTIR", y = "Number of Particles", x = "Measured Particle Length (µm)", alpha = "Identification")+
   scale_alpha_manual(values = c(1,0.25)) +
