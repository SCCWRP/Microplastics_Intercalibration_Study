...
Join_Microscopy_Total <- inner_join(Results_Clean_Microscopy, totalspikecol, by = c("sampleid", "sampletype_f")) %>% #Join total_spike column to microscopy results data frame
    ...
    group_by(labid, sampletype_f, sampleid, plastic_f, microscopy_f, total, total_spike, total_pct_recovery, expertise) %>% 
    summarise()
...
sizefractionsumresults <- Results_Clean_Microscopy %>% 
  ...
  group_by(labid, sampletype_f, sampleid, microscopy_f, sizefraction_f, total_sizefraction, expertise) %>% 
  summarise()
...
colorsumresults <- Results_Clean_Microscopy %>% 
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blank samples from results
  group_by(labid, sampletype_f, sampleid, microscopy_f, color_f, total_color, expertise) %>% 
  summarise()
...
morphologysumresults <- Results_Clean_Microscopy %>% 
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blank samples from results
  group_by(labid, sampletype_f, sampleid, microscopy_f, morphology_f, total_morphology, expertise) %>% 
  summarise()
...
polymersumresults <- Results_Clean_Microscopy %>% 
  filter(!grepl("4", sampleid)) %>%
  filter(!grepl("8", sampleid)) %>% #Dropping blank samples from results
  group_by(labid, sampletype_f, sampleid, microscopy_f, polymer_f, total_polymer, expertise) %>% 
  summarise()
...
#Combination - Color and Morphology Only
 ...
 combosumresultscm <- Results_Clean_Microscopy %>%
   filter(!grepl("4", sampleid)) %>%
   filter(!grepl("8", sampleid)) %>% #Dropping blank samples from results
   group_by(labid, sampletype_f, sampleid, microscopy_f, color_f, morphology_f, total_color_morphology, expertise) %>%
   summarise()
...
#Combination - Size Fraction, Color and Morphology 
 ...
 combosumresults <- Results_Clean_Microscopy %>%
   ...
   filter(!grepl("8", sampleid)) %>% #Dropping blank samples from results
   drop_na(sizefraction_f) %>% #Drop particles not assigned to a size fraction
   group_by(labid, sampletype_f, sampleid, microscopy_f, sizefraction_f, color_f, morphology_f, total_color_morphology_sizefraction, expertise) %>%
   summarise()
...
a <- Join_Microscopy_Color %>%
  ...
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, microscopy_f, color_f, color_pct_recovery) %>% 
  summarise() %>% 
...
m <- Join_Microscopy_Color %>%
  ...
  ungroup() %>% 
  select(sampletype_f, labid, color_f, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  rename("Matrix" = sampletype_f) %>%
  rename("Lab ID" = labid) %>%
  rename("Color" = color_f) %>%
  distinct() 
...
n <- m %>%
  ...
  mutate(`Max (%)` = max(`Mean (%)`)) %>%
  ungroup() %>% 
  select(`Matrix`, `Color`, `Global Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  distinct() 
...
a <- Join_Microscopy_Morphology %>%
  ...
  filter(plastic_f != "N") %>%
  #Data manipulation
  group_by(sampletype_f, labid, sampleid, microscopy_f, morphology_f, morphology_pct_recovery) %>% 
  summarise() %>% 
...
kable(h, digits = 2, caption = "Microscopy: Total Particle Recovery by Morphology. Summary statistics by sample (n = 1-3 samples). Not blank corrected.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "700px", height = "200px")
...
i <- Join_Microscopy_Polymer %>%
  ...
  ungroup() %>% 
  select(sampletype_f, labid, polymer_f, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  rename("Matrix" = sampletype_f) %>% 
  rename("Lab ID" = labid) %>%
  rename("Polymer" = polymer_f) %>%
  distinct() 
...
j <- Join_Microscopy_Polymer %>%
  ...
  mutate(`Max (%)` = max(polymer_pct_recovery)) %>%
  ungroup() %>% 
  select(sampletype_f, polymer_f, `Mean (%)`, `Median (%)`, `SD (%)`, `Min (%)`, `Max (%)`) %>%
  rename("Matrix" = sampletype_f) %>%
  rename("Polymer" = polymer_f) %>%
  distinct() 
...
```{r echo = FALSE, error = FALSE, warning = FALSE, message = FALSE, results = 'asis', fig.width = 12, fig.height = 10}

#Make data frame of particle characteristics
df <- Join_Microscopy_Combo %>%
  filter(plastic_f == "Y") %>% 
  group_by(morphology_f, color_f, sizefraction_f) %>% 
  summarise()
...
a <- Join_Microscopy_Combo %>%
  group_by(labid, sampleid, microscopy_f,sizefraction_f, color_f, morphology_f, color_morphology_sizefraction_pct_recovery) %>% 
...
b <- Join_Microscopy_Combo %>%
  ...
  ungroup() %>%
  group_by(labid, microscopy_f,color_morphology_sizefraction_pct_recovery_mean) %>%
  summarise() %>%
  #Creating plot
...
c <- Join_Microscopy_Combo %>%
  group_by(labid, sizefraction_f, color_f, morphology_f) %>%
  filter(sizefraction_f == df$sizefraction_f[[i]]) %>% 
  filter(color_f == df$color_f[[i]]) %>% 
  filter(morphology_f == df$morphology_f[[i]]) %>%
  ungroup() %>%
  select(labid, sampleid, color_morphology_sizefraction_pct_recovery) %>%
  rename("Lab ID" = labid) %>%
  rename("Sample ID" = sampleid) %>%
